...
function login($usuario, $password, $con, $proceso){
    $sql = $con->prepare("SELECT id, usuario, password, id_cliente FROM cuentas WHERE usuario LIKE ? LIMIT 1");
    $sql->execute([$usuario]);
...
function esActivo($usuario, $con){
    ...
    $sql = $con->prepare("SELECT activacion FROM cuentas WHERE usuario LIKE ? LIMIT 1");
    $sql->execute([$usuario]);
...
function solicitaPassword($user_id, $con){
    ...
    $sql = $con->prepare("UPDATE cuentas SET token_password = ?, password_request = 1 WHERE id = ? ");
...
function verificaTokenRequest($user_id, $token, $con){
    $sql = $con->prepare("SELECT id FROM cuentas WHERE id=? AND token_password LIKE ? AND
...
function actualizaPassword($user_id, $password, $con){
    $sql = $con->prepare("UPDATE cuentas SET password= ?, token_password = '', password_request
    = 0 WHERE id = ?");
